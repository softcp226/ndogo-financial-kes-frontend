...
document.querySelector("#validate_email").addEventListener("click", async (e) => {
 ...
 try {
  ...
  const response = await fetch("https://ndogo-financial-kes-production.up.railway.app/api/user/transfer/validate", {
    method: "POST",
    headers: {
...
document.querySelector("#transfer_fund").addEventListener("click", async (e) => {
  ...
  try {
    const response = await fetch("https://ndogo-financial-kes-production.up.railway.app/api/user/transfer/fund", {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
      },
