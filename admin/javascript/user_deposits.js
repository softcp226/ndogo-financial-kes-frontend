...
const createAndAppendElement = (element) => {
  ...
  depositor.innerHTML = element.user
    ...
    : "";
  currency.innerHTML = element.transaction.transaction_date;
  amount.innerHTML = `KSH${element.deposit_amount
    .toString()
    .replace(/\B(?=(\d{3})+(?!\d))/g, ",")}`;

  proof_evidence.innerHTML = element.receipt
    ? `<a href="${element.receipt}" target="_blank">View Receipt</a>`   
      : "No receipt provided";
